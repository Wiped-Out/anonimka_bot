...
async def reply_to_message(message: types.Message, state: FSMContext):
    async with state.proxy() as fsm_data:
        try:
            reply_to_message_id = fsm_data['message_id']
        except KeyError:
            reply_to_message_id = None

    await state.reset_state(with_data=True)
    await message.send_copy(chat_id=settings.CHANNEL_ID, reply_to_message_id=reply_to_message_id)
    await message.answer(messages.POST_SENT)


@dp.message_handler(content_types=types.ContentType.ANY, state='*')
async def messages_handler(message: types.Message, state: FSMContext):  # noqa: WPS217
    ...
    if not (message.forward_from or message.forward_from_chat):
        await message.send_copy(chat_id=settings.CHANNEL_ID)
        return await message.answer(messages.POST_SENT)

    # Сообщение переслано из анонимки, значит, это реплай
    if message.forward_from_chat and message.forward_from_chat.id == settings.CHANNEL_ID:
        async with state.proxy() as fsm_data:
            fsm_data['message_id'] = message.forward_from_message_id
    ...
    await message.forward(chat_id=settings.CHANNEL_ID)
